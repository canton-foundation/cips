...
* Section 4 covers the changes in interfaces and the new capabilities made possible.
  * 4.1 covers the use cases and reasons for the enhancements.
  * 4.2 covers the changes to interfaces and guidelines.
* Section 5 covers cross-version compatibility with the V1 standards.
* Section 6 covers how Canton Coin will implement token standard V2 in a fully backwards compatible way.
* Section 7 covers planned examples and demonstrations in integration tests, that will illustrate how to implement a token for the V2 standards, and how wallets and trading and settlement venues should integrate with the standard(s).
...
| :---- | :---- | :---- | :---- | :---- |
| V1 | V1 | V1 |  | As per CIP-0056 |
| V1 | V1 | V2 |  | As per CIP-0056 |
| V1 | V2 | V1 |  | As per CIP-0056 |
| V1 | V2 | V2 |  | The sender has no means of setting receiver account info. The V2 asset could use the `TransferPendingInternalWorkflow` status to insert a step for setting the receiver account, OR resolve the receiver account in the factory or accept choices. |
| V2 | V1 | V1 |  | As per CIP-0056 |
| V2 | V1 | V2 |  | Use V2 factory with basicAccount as receiver |
| V2 | V2 | V1 |  | As per CIP-0056 |
| V2 | V2 | V2 |  | Use V2 throughout |
...
| V1  | V1  | V1 | V1 |  | As per CIP-0056 |
|  |  |  | V2 |  | As per CIP-0056. Asset emits V1 Allocation as called through V1 factory. Wallets are V1 so only use basic accounts. |
|  |  | V2 | V1 |  | As per CIP-0056. Wallet calls V1 factory choice. |
|  |  |  | V2 |  | The same limitation applies as for a transfer from a V1 to a V2 wallet on a V2 asset. The app has no notion of accounts so the asset has to resolve accounts somewhere in the flow. |
| V2  | V1 | V1 | V1 |  | Emit V1 `AllocationRequest`. Then as per CIP-0056. |
|  |  |  | V2 |  | Emit V1 or dual-version `AllocationRequest`. Then as per CIP-0056. Traders with V2 wallets can use accounts, and the app can set them on the transferLeg metadata to ensure that V1 wallets create the right allocations. |
|  |  | V2 | V1 |  | Emit V1 `AllocationRequest`. Then as per CIP-0056. |
|  |  |  | V2 |  | App should behave *as if* wallets are V1 to get maximum compatibility; i.e.,  Emit V1 or dual-version `AllocationRequest`. Then as per CIP-0056.  |
|  | V2 | V1 | V1 |  | V2 settlement on V1 Asset is not possible. Apps should use the advertised asset compatibility (5.3) to avoid this case. |
|  |  |  | V2 |  | Flow via `AllocationRequest` is not possible. See section 5.6 for a path via dApp API. |
|  |  | V2 | V1 |  | V2 settlement on V1 Asset is not possible. Apps should use the advertised asset compatibility (5.3) to avoid this. |
|  |  |  | V2 |  | Use V2 flow. |
