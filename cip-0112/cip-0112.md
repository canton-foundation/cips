...
* `splice-token-standard-utils` for shared types and functions, including those shared between versions and used for cross-version compatibility.
...
CIP-0112 introduces a new `EventLog` interface, which emits standalone events as non-consuming choices that are free of side-effects, similar to ERC-20 events. This simplifies parsing for wallets, exchanges and apps: just look for `EventLog_HoldingsChange` events to understand all changes to the holdings of an account.
...
CIP-0112 now adds features that allow important variations of this flow. 
...
There are scenarios where wallets or apps want to call multiple token standard actions in a single transaction. For example, to take a single UTXO of 100.0 and transfer 1.0 amount to 10 recipients. This was already possible with CIP-0056, but required deployment of custom smart contract code. CIP-0112 adds standardised batching logic that is deployed to all Splice validator nodes as part of the `splice-util-token-standard-wallet` package. This allows all common token standard actions to be performed in batches, with optimal use of input holdings.

## 4.2 UX/Flow Example In Detail

The biggest change in flow are the changes to `Allocation`-based trade settlement described in [4.1.1](#411-privacy-enhanced-batch-settlement) and [4.1.2](#412-improved-user-flows-with-trusted-venues), respectively.
To illustrate the intended settlement flow, this section walks through a trade using the enhancements described [4.1.1](#411-privacy-enhanced-batch-settlement), illustrating privacy and a UX like [4.1.2](#412-improved-user-flows-with-trusted-venues).
...
The app now emits four `AllocationRequests`:
   ...
   4. For itself, the App, *optionally* (see [4.3.8](#438-guidelines--interfaces-for-performance-optimization)) with
      ```
      [
        TransferLeg with sender=alice; receiver=app; amount=0.1; instrumentId=usd,
...
The App now takes all 7 allocations and in the same transaction sticks them into two choices `SettlementFactory_SettleBatch`, one for the X `SettlementFactory` and one for the \$ `SettlementFactory`, with just the App as `actor`, as specified in the `SettlementInfo`:
   1. 3.a,c,e into the X `SettlementFactory`
   2. 4.b,d,f,g into the \$ `SettlementFactory`

The allocations match in the sense of [4.3.7](#437-app-wallet-and-asset-guidelines-for-allocations), so both settlements should work.
...
The type `FinalizedAllocation` will be discussed in [section 4.3.5](#435-committed-allocations-and-iterated-settlement). For the purposes of this section, it is equivalent to a `ContractId Allocation`.
...
The `basicAccount` function (provided by the `splice-token-standard-utils` package, see [5.3](#53-standard-mappings-and-choice-implementations)) is a convenience function to express an `Account` that is equivalent to the plain `Party` in the original CIP-0056 standards. For accounts that match that shape, V2 visibility and authorization rules MUST match those of V1, meaning the owner calls all factory choices and the choices on the instructions.
...
## 4.3.4 EventLog for Transaction Parsing
...
At the core, the call arguments consist of the input and output holding `ContractId`s, and the transfers that resulted in these changes. The `ContractId` deltas allow efficient updates of indices. `HoldingView`s are not included for space efficiency. The `HoldingView`s can be read from the actual create and archive events of the holdings in the same transaction, or retrieved from separate endpoints on the Ledger API.
...
## 4.3.5 Committed Allocations and Iterated Settlement
...
### 4.3.5.1 Topping up Allocations

`Allocations` with iterated settlement can be rebalanced or merged using the standard settlement flow. For example, say an `Account` has a Canton Coin deposit to an on-chain exchange in the form of an `Allocation` with `nextIterationFunding` containing 100.0 CC. They can create a second such allocation with 500.0 CC. The exchange can now call `SettlementFactory_SettleBatch` with the following parameters:

1. On the `Allocation` with 100.0:
  1.1 `extraTransferLegSides = [TransferLegSide "merge" SenderSide traderAccount 100.0]`
  1.2 `nextIterationFunding = None`
2. On the `Allocation` with 500.0:
  1.1 `extraTransferLegSides = [TransferLegSide "merge" ReceiverSide traderAccount 100.0]`
  1.2 `nextIterationFunding = Map.FromList [("Amulet", 600.0)]`

The result will be a new `Allocation` with with 600.0 CC funding.

### 4.3.5.2 Requesting Allocations

Since allocations can now have significantly different properties in terms of behaviour related to `committed` and `nextIterationFunding`, it's possible that an app may want to request multiple allocations from a single trader even for a single `admin`. To allow for this, `AllocationRequest` now represents the request for a list of distinct allocations through a list of `AllocationSpecifications`:
...
## 4.3.6 Metadata for Paused Status
...
## 4.3.7 App, Wallet, and Asset Guidelines for Allocations
...
5. Allocations that lock holdings, like Canton Coin Allocations, SHOULD only lock the *net* amounts needed by the trader to fund the settlement, including the amounts on `nextIterationFunding`.

The reason for these recommendations is that it makes it tractable for wallets, assets and apps to perform a complex settlement with privacy and good UX as outlined with [4.1.1](#411-privacy-enhanced-batch-settlement) and [4.1.2](#412-improved-user-flows-with-trusted-venues).

## 4.3.8 Guidelines & Interfaces for Performance Optimization

The V2 interfaces are structured to allow assets to run the V2 settlement flows in a performance optimized fashion, both if they require the more fine-grained privacy enabled by [4.3.1](#431-configurable-executors-and-batch-settlement-via-settlementfactory), and also if transaction data can be made visible to anyone as is the case with Canton Coin.
...
If the interaction between app and wallet can be kept off-ledger, the `AllocationRequests` can be elided. Instead, a CIP-0103 flow can be used to go straight to transaction \#2. As a result, Transaction \#1 and the `AllocationRequest_Accept` child views in transactions #2 to #4 can be removed, saving 1 transaction and 7 views.
...
1. Neither the assets (X and \$) attempt to preserve privacy between traders. This allows for [4.3.8.3](#4383-optimizations-for-assets-with-reduced-privacy).
2. The settlement itself can be authorized by all traders \- ie Alice, Bob, and Carol have all pre-authorized settlement in some way. This allows for [4.3.8.4](#4384-eliding-allocations-for-settlement-authorizers).
3. Alice, the only party that needs to fund the settlement, is able to use a CIP-0103 signing flow. This allows for [4.3.8.2](#4382-optimizations-for-ui-driven-flows).

[TX1](#422-emit-allocation-requests-tx1) is eliminated by [4.3.8.2](#4382-optimizations-for-ui-driven-flows).
[TX2](#4231-alices-allocations-tx2) collapses to a single view to directly call `AllocationFactory_Allocate` due to the X Allocation view and root node being eliminated by [4.3.8.4](#4384-eliding-allocations-for-settlement-authorizers).
[TX3](#4232-bobs-allocations-tx3)-[TX5](#4234-apps-allocations-tx5) are eliminated by [4.3.8.4](#4384-eliding-allocations-for-settlement-authorizers).
...
* Based on [4.3.8.3](#4383-optimizations-for-assets-with-reduced-privacy), both the X and \$ apps create single views for `SettlementFactory_SettleBatch`.

Total: 2 Transactions; 4 Views

### 4.3.9 Batch Utilities

The package `splice-util-token-standard-wallet` adds a new module `Splice.Util.Token.Wallet.BatchingUtilityV2` with a single template that allows for batched execution of CIP-0112 actions:
...
* Apps
  ...
  * Apps SHOULD make sure that any `AllocationSpecifications` in `AllocationRequests` that reference V1-only assets can be mapped to V1 `Allocations`. This is made possible by assets advertising their compatibility. See [5.2](#52-advertising-compatibility).
    ...
    * `nextIterationFunding` MUST be `None`.
...
As specified by CIP-0056, assets advertise their token standard compatibility via the `registry/metadata/v1/instruments` API. For V2-compatible assets, the `supportedApis` field will now also include the V2 packages listed under [Section 3](#3-specification).
...
3. The optimizations [4.3.8.3](#4383-optimizations-for-assets-with-reduced-privacy)\.
...
4. Is used in Daml Script-based integration and compatibility testing relative to [Section 4.3.8](#438-guidelines--interfaces-for-performance-optimization) and [Section 5](#5-backwards-compatibility)\.
...
* **2026-06-12:** Approval announced via [mailing list thread](https://lists.sync.global/g/cip-announce/topic/119772687)
* **2026-09-11:** Editorial revision without changes to the specified interfaces:
  * Corrected the package names in [Section 3](#3-specification) to `splice-api-token-allocation-v2`,
    `splice-token-standard-utils`, and `splice-util-token-standard-wallet`, and standardized the
    `V1`/`V2` version notation throughout.
  * Renamed `nextIteratedSettlement` to `nextIterationFunding`, `extraTransferLeg` to
    `extraTransferLegSides`, and `transferLeg id` to `transferLegId` in the prose and examples of
    [Section 4.3.5](#435-committed-allocations-and-iterated-settlement) and [Section 5.1](#51-compatibility-rules)
    to match the interface definitions.
  * Renumbered the sections 4.3.4 to 4.3.7 to remove the duplicate 4.3.8 heading.
  * Replaced plain section numbers with links and corrected broken section anchors.
  * Fixed typos and clarified wording in [Section 4.1.5](#415-improved-transaction-parsing),
    [Section 4.2.4](#424-settlement-tx6), and [Section 4.3.8.2](#4382-optimizations-for-ui-driven-flows).
  * Added the Copyright and Changelog sections.
