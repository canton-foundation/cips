...
Having executed a trade off-chain, the App app defines the `SettlementReference` with `executors=[app]` and a list of nine trading legs:
...
The App now takes all 7 allocations and in the same transaction sticks them into new choices `SettlementFactory_SettleBatch` of the X and \$ `SettlementFactories` with just the App as `actor` as specified in the `SettlementReference`:
...
* `SettlementReference.executors` remains of type `[Party]`
...
The timing parameters `settleBefore` and `allocateBefore` on `SettlementInfo` have been reworked and moved to different types. `SettlementInfo` is renamed to `SettlementReference` to indicate more clearly that that's what should be matched on across `Allocations`.
...
The `SettlementReference` type is reduced to this:
...
1. A settlement is specified by an app by setting `SettlementReference`, and a list of `AllocationSpecifications` including `TransferLegs`. The `Accounts` appearing as `sender` or `receiver` on a transfer leg are collectively called the _trading accounts_.
   * Apps MUST ensure that the `SettlementReference` identifier of the settlement is unique for all settlements with the same `executors`, so that the `SettlementReference` can be used to reliably match allocations with allocation requests in UIs and backends.
...
3. Wallets SHOULD create exactly one `Allocation` per `RequestedAllocation` on a received `AllocationRequest`.
   * In terms of UX, they should ask the user to authorize the settlement as a whole with one signature, showing the `SettlementReference` and *all* transfer legs.
   * Wallets MUST create the `Allocation`s for the *exact same* `SettlementReference` including metadata as is specified in the `AllocationRequest`.
   * Wallets SHOULD display the whole `SettlementReference` including `metadata` to ensure users are aware of the settlement for which they are creating the allocation.
...
4. Assets SHOULD ensure that if there is a set of matching `Allocations`, they are guaranteed to be settleable through the `SettlementFactory_SettleBatch` choice. Matching means identical `SettlementReference`, and every transfer leg appears once on the sender `Allocation` and once on the receiver `Allocation`.
