...
1. Creates a vote request to action “Offboard Member” and
2. Configures the “Offboard Member” action with the SV Party ID whose weight should be reduced to 0 ”Reduced SV”.
3. Notifies each SV Operator to update their approved SV Onboarding config to remove the SV.
...
5. Take the concurrent amuletToIssuePerYear / ( 365 * 144 ) as the maximum number of CC available to be minted; referred to as max_mints
